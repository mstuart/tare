...
assert.strictEqual(
  basic.baseURL,
  "http://127.0.0.1:8787",
  "withTare() baseURL default"
);
assert.strictEqual(
  basic.base_url,
  "http://127.0.0.1:8787",
  "withTare() base_url default"
);
...
assert.strictEqual(
  merged.baseURL,
  "http://127.0.0.1:9001",
  "withTare custom port baseURL"
);
assert.strictEqual(
  merged.base_url,
  "http://127.0.0.1:9001",
  "withTare custom port base_url"
);
...
mw.transformParams({ params: { model: "claude-3-5-sonnet" } }).then(
  (result) => {
    assert.strictEqual(
      result._tare_base,
      "http://127.0.0.1:8787",
      "transformParams sets _tare_base"
    );
  }
);
