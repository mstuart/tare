//! Library entry points for the `tare` CLI: run the compression pipeline and produce a fidelity report.

use serde::Deserialize;
use tare_core::emit::{emit, FidelityReport};
...
pub fn run_compress_with_budget(
    blocks_json: &str,
    task: &str,
    budget: Option<u32>,
    ...
    let plan =
        Planner::new(passes).plan_with_budget(&segs, &SessionState::default(), &task_sig, budget);
    let (emitted, report) = emit(&segs, &plan);

    let compressed = emitted
        .iter()
...
fn parse_kind(kind: &str, class: &Option<String>) -> Result<SegmentKind, String> {
    Ok(match kind {
        ...
        "tool_output" => SegmentKind::ToolOutput {
            class: class.clone().unwrap_or_else(|| "unknown".into()),
        },
...
mod tests {
    ...
    fn run_compress_with_budget_evicts_to_fit() {
        ...
        assert!(
            budgeted.report.net_tokens <= 8
                || budgeted.report.net_tokens < unbudgeted.report.net_tokens,
    ...
    fn parses_roles_and_kinds_including_tool_output_class() {
        ...
        assert!(
            matches!(&blocks[1].kind, SegmentKind::ToolOutput { class } if class == "cargo-test")
        );
