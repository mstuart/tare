...
fn main() {
    ...
    println!(
        "Cull benchmark — budget {budget} tokens, {} items\n",
        corpus().len()
    );
    print!("{}", render_board(&board));
    println!("\nsaved% = tokens removed (the RTK/Headroom headline); time/call = cost, same clock for all.");
    println!(
        "Cull is in-process; incumbents run via the ShellCompressor seam (subprocess), so their"
    );
    println!(
        "time/call includes per-call model load. Warm steady-state (model resident): LLMLingua-2"
    );
    println!(
        "≈273ms/call, Headroom abstains on small inputs. Cull's 100% fidelity makes its savings"
    );
