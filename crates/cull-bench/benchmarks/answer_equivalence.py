#!/usr/bin/env python3
"""Local-LLM answer-equivalence + columnar-readability validation for Cull.

Measures two things:
1. Answer equivalence: does Cull-compressed context preserve the LLM's ability to answer
   factual questions, vs the original context and headroom-compressed context?
2. Columnar readability: can the LLM read Cull's ⟪jc1⟫ columnar format and extract values?

Requirements:
  - /tmp/cull-headroom-venv313/bin/python (transformers + torch + headroom-ai + tiktoken)
  - /Users/mark/git/cull/target/release/cull binary

Run:
  /tmp/cull-headroom-venv313/bin/python answer_equivalence.py
"""
...
def run_answer_equivalence(tokenizer, model, model_id: str):
    ...
    for i, case in enumerate(cases):
        ...
        print(
            ...
            f"  orig={hits['original']} cull={hits['cull']} head={hits['headroom']}"
        )
...
def run_columnar_readability(tokenizer, model, model_id: str):
    ...
    for i, case in enumerate(COLUMNAR_CASES):
        ...
        columnar = cull_compress(raw_json, case["query"])
...
def main() -> int:
    ...
    print(f"  cull:      {correct['cull']}/{n} = {correct['cull']/n:.0%}")
    print(f"  headroom:  {correct['headroom']}/{n} = {correct['headroom']/n:.0%}")
    cull_ratio = sum(ratios['cull']) / n if n else 0.0
    head_ratio = sum(ratios['headroom']) / n if n else 0.0
    print(f"\nMean compression ratio:")
    print(f"  cull:      {cull_ratio:.1%}")
    print(f"  headroom:  {head_ratio:.1%}")
    print()

    # Verdict
    cull_acc = correct['cull'] / n
    orig_acc = correct['original'] / n
    if cull_acc >= orig_acc:
        print(f"VERDICT: Cull PRESERVES task accuracy ({cull_acc:.0%} vs original {orig_acc:.0%})")
    else:
        print(f"VERDICT: Cull DEGRADES task accuracy ({cull_acc:.0%} vs original {orig_acc:.0%}) — CONCERN")
