...
mod tests {
    ...
    fn replace_is_lossless_accepts_valid_delta_rejects_bad_one() {
        ...
        let by_id: HashMap<SegmentId, &Segment> = [(base.id, &base), (target.id, &target)]
            .into_iter()
            .collect();
        ...
        let good = SegmentAction::Replace {
            rendered: good_patch.into_bytes(),
            token_count: 2,
