...
pub fn compact(text: &str) -> Option<String> {
    ...
    let is_entity = |w: &str, bare: &str| {
        w.chars().next().is_some_and(char::is_uppercase) && !STOPWORDS.contains(&bare)
    };

    // base keep decision per word
    let bares: Vec<String> = words
        .iter()
        .map(|w| {
            w.chars()
    ...
    for i in 1..words.len() {
        if keep[i - 1]
            && (is_num(words[i - 1]) || is_entity(words[i - 1], &bares[i - 1]))
            && !bares[i].is_empty()
        {
            keep[i] = true;
        }
