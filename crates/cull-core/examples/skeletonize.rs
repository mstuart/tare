...
//!
//! Run with:
//!     cargo run -p cull-core --example skeletonize -- path/to/file.rs
//!
...
fn main() {
    let path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "crates/cull-core/examples/skeletonize.rs".to_string());
    ...
    match cull_core::code_skeleton::skeletonize(&src, &path) {
        Some(skeleton) => {
