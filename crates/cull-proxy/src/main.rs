...
async fn main() {
    let upstream =
        std::env::var("CULL_UPSTREAM").unwrap_or_else(|_| "https://api.anthropic.com".into());
    let port: u16 = std::env::var("CULL_PORT")
        .ok()
        .and_then(|p| p.parse().ok())
        .unwrap_or(8787);
    let recency_keep: usize = std::env::var("CULL_RECENCY")
        .ok()
        .and_then(|p| p.parse().ok())
        .unwrap_or(4);
    let enabled = std::env::var("CULL_ENABLED")
        .map(|v| v != "0" && v != "false")
        .unwrap_or(true);
