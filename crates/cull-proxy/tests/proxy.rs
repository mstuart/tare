...
async fn proxy_response_carries_cull_report_headers() {
    ...
    assert!(
        resp.headers().get("x-cull-net-tokens").is_some(),
        "response carries x-cull-net-tokens header; got headers: {:?}",
        resp.headers()
    );
}
...
async fn skeletonizes_code_via_controller_under_high_fill() {
    ...
    assert!(
        received.contains("[cull: code skeleton"),
...
async fn verbosity_spike_backs_off_compression_end_to_end() {
    ...
    assert!(
        spike_header,
        "turn 5 response carries x-cull-verbosity-spike"
    );
}
