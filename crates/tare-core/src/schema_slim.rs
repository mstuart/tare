...
//!
//! This is the one place Tare does a **lossy** transform, so it is deliberately separate from the
//! lossless `compress` pipeline and exposed only via an explicit entry point (`tare slim-schema`).
