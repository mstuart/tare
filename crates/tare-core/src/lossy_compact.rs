//! Opt-in LOSSY compaction of large JSON arrays — Tare's aggressive mode, off by default.
//!
...
//! provides the same lossy lever, opt-in and explicit (`tare compact-lossy`), so Tare can match or
...
//! the uniform bulk, and append an explicit `[tare-lossy: M of N rows elided]` marker. Kept rows are
...
///
/// Together they let Tare match or beat a tabular filter's ratio while still preserving the
...
fn compact_text(
    ...
    let out = format!(
        "{}\n[tare-lossy: {dropped} of {n} {label} elided; kept boundary+alerts+relevant{note}]",
        kept.join(joiner)
    );
    if out.len() < text.len() {
        Some(out)
...
fn compact_json_array(value: &Value, boundary: usize, syms: &Syms) -> Option<String> {
    ...
    out.push_str(&format!(
        "\n[tare-lossy: {dropped} of {n} uniform rows elided; kept boundary+anomalies]"
    ));
...
mod tests {
    ...
    fn max_rows_caps_kept_lines_but_never_drops_alerts() {
        ...
        let kept: Vec<&str> = out
            .lines()
            .filter(|l| !l.starts_with("[tare-lossy"))
            .collect();
