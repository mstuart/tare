...
impl Default for EnvelopeDedupPass {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Pass for EnvelopeDedupPass {
    ...
    fn propose(&self, ctx: &PlanCtx) -> Vec<PlanEntry> {
        ...
        for (idx, s) in outs.iter().enumerate() {
            let Ok(s_str) = std::str::from_utf8(&s.bytes) else {
                continue;
            };
            let mut best: Option<(crate::segment::SegmentId, String, u32)> = None;
            for base in &outs[..idx] {
                let Ok(b_str) = std::str::from_utf8(&base.bytes) else {
                    continue;
                };
            ...
            if let Some((base, patch, pt)) = best {
                if (pt as f64) < self.min_reduction * (s.token_count as f64) {
                    proposals.push(PlanEntry {
                        id: s.id,
                        action: SegmentAction::Replace {
                            rendered: patch.into_bytes(),
                            token_count: pt,
                            reconstruct: Reconstruct::Delta { base },
...
mod tests {
    ...
    fn repetitive_envelope_is_deltad_against_earlier_output() {
        ...
        let plan = Planner::new(vec![Box::new(EnvelopeDedupPass::new())])
        ...
        assert!(
            ...
            "near-duplicate envelope deltad"
        );
    }
    ...
    fn dissimilar_outputs_are_not_deltad() {
        let a = out(
            ...
            0,
            60,
            "completely different alpha content here and there everywhere",
        );
        let b = out(
            1,
            1,
            60,
            "nothing in common beta gamma delta epsilon zeta eta theta",
        );
        let plan = Planner::new(vec![Box::new(EnvelopeDedupPass::new())])
