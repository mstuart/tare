...
pub struct CanonicalFileStore {
    map: HashMap<String, CanonicalFile>,
}

impl CanonicalFileStore {
    pub fn get(&self, path: &str) -> Option<&CanonicalFile> {
        self.map.get(path)
    }
...
#[derive(Debug, Default)]
pub struct ToolClassRegistry {
    latest: HashMap<String, ToolRun>,
}

impl ToolClassRegistry {
    pub fn record(&mut self, class: &str, turn: usize, exit_code: Option<i32>) {
        self.latest
            .insert(class.to_string(), ToolRun { turn, exit_code });
    }
    pub fn latest_run(&self, class: &str) -> Option<ToolRun> {
        self.latest.get(class).copied()
    }
}
...
pub struct SpanLedger {
    pub entries: HashMap<SegmentId, RefLedger>,
}

/// Cache-prefix commitment (spec Rule 1/7): the frozen zone is a content hash.
#[derive(Debug, Default)]
pub struct CachePrefixCommitment {
    pub frozen_hash: Option<u64>,
    pub frozen_len_tokens: usize,
}
